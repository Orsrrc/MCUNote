...
`RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE)` 打开APB2总线上GPIOA外设的时钟
...
四引脚和七引脚的OLED硬件电路，通常4引脚的用i^2^c通信协议，七引脚用于spi通信协议

![image-20260113150042700](./MCU.assets/image-20260113150042700.png)

模块提供的OLED.c、OLED.h以及OLED_Font.h放在Hardware下面

模块提供函数，及其用法。注意，函数中的参数通常是指定为其实行列，然后根据字符串长度自行打印，，而不是指定起始地址和末尾地址。

![image-20260113150304470](./MCU.assets/image-20260113150304470.png)

OLED中如果不带字库，需要我们复制一个字库进来，然后使用。在模块中为OLED_Font.h



## Keil调试模式

选择硬件上调试模式，注意此时需要将STLink与STM32都连接好

![image-20260113152053928](./MCU.assets/image-20260113152053928.png)

然后点击启动调试模式，进入调试模式启动。

![image-20260113152113962](./MCU.assets/image-20260113152113962.png)

![image-20260113152333590](./MCU.assets/image-20260113152333590.png)

## 中断系统



![image-20260113153249291](./MCU.assets/image-20260113153249291.png)



中断程序执行流程

通常情况下主函数循环执行，不断检查是否收到了中断信号，然后调用对应的中断函数

![image-20260113153501203](./MCU.assets/image-20260113153501203.png)
...
因为我们定义的中断处理函数是不固定的，而硬件（单片机）上只能跳转到一个固定的地址，因此当产生中断以后，就需要去该地址里面查找对应的处理函数，使得中断由一个固定的地址跳转到不固定的函数处理地址。

该地址存储在中断向量表里面。

![image-20260113153957751](./MCU.assets/image-20260113153957751.png)
...
![image-20260113154641180](./MCU.assets/image-20260113154641180.png)



NVIC会将抢占优先级和响应优先级进行分组，其中值越小，优先级越高

![image-20260113154847929](./MCU.assets/image-20260113154847929.png)

### EXTI

![image-20260113155441068](./MCU.assets/image-20260113155441068.png)



![image-20260113160311593](./MCU.assets/image-20260113160311593.png)



![image-20260113160459575](./MCU.assets/image-20260113160459575.png)





![image-20260113161359166](./MCU.assets/image-20260113161359166.png)

这里的中断屏蔽寄存器下面的与门作用为一个开关。当中断屏蔽寄存器输出1时，与上任意数为任意数，即1&x = x ，而输出0时为0，代表是否允许中断。
...
![image-20260113161813624](./MCU.assets/image-20260113161813624.png)

由于该外部设备的输入是突发的，STM32的接收是被动的，因此可以考虑采用外部中断的方式，当接收到信号以后，发出中断信号，然后对中断信号进行处理。

硬件电路

![image-20260113162851904](./MCU.assets/image-20260113162851904.png)



代码

先要在总线上打开该外设的时钟，否则外设无法工作

如果不确定某一个外设是接在哪个总线上的，可以通过函数的定义跳转到对应的库函数里面去查看是否有外设的参数。

其中EXTI与NVIC不需要开启时钟  NVIC是内核的外设，内核的外设不需要开启时钟

![image-20260113164831245](./MCU.assets/image-20260113164831245.png)

对于某一个外设，如果不知道它需要设置为什么样的输入模式，可以去手册里面看GPIO的引脚推荐配置。





![image-20260113165329870](./MCU.assets/image-20260113165329870.png)



![image-20260113170711292](./MCU.assets/image-20260113170711292.png)
